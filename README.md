...
Project Pipeline
Data Cleaning — Python / pandas (Customer_Behaviour_Analysis.ipynb)
...
Analysis — SQL / MySQL (customer_shopping_behavior.sql)
...
How to Reproduce
Clean the data: run Customer_Behaviour_Analysis.ipynb top to bottom in a Python environment with pandas, sqlalchemy, and pymysql installed.
Load to MySQL: update the connection credentials in the notebook's last cell, then run it to create the customer_behaviour database and customer table.
...
Run the analysis: execute the queries in customer_shopping_behavior.sql against the loaded table (MySQL client or Workbench).
View the dashboard: open customer_shoppping.pbix in Power BI Desktop.
Requirements
